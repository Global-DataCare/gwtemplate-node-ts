...
## Preserve persistent identity wire formats

- Treat every persisted or externally issued DID, URN, multibase value,
  multihash, CID, index key and normalization rule as an immutable wire
  contract. Never change its discriminator, preimage, digest algorithm,
  multicodec or byte encoding under the existing format name.
- Before recovering or changing identity behavior, inspect the last deployed
  implementation, its original helper and the oldest committed data fixtures.
  Reuse those helpers and fixtures in the first red test; do not replace a
  historical vector with an expected value generated by the proposed helper.
- Make the compatibility test prove the exact historical serialized value and
  that it still resolves, authorizes and retrieves the same stored subject.
  Also prove the current format separately. Unit mocks do not replace one real
  local cross-system journey containing a pre-change identifier.
- Introduce a stronger digest or normalization only as a new versioned format.
