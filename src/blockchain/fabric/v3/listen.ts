...
import { channelName } from './fabric-config';
  ...
  try {
    ...
    console.log(
      `Starting event listening from block ${
        checkpointer.getBlockNumber() ?? startBlock
      }`
    );
    console.log(
    ...
    if (simulatedFailureCount > 0) {
      console.log(
        `Simulating a write failure every ${simulatedFailureCount} transactions`
      );
    }
...
class BlockProcessor {
  ...
  async process(): Promise<void> {
    ...
    const validTransactions = this.#getNewTransactions().filter((transaction) =>
      transaction.isValid()
    );
    ...
    for (const transaction of validTransactions) {
      ...
      await this.#checkpointer.checkpointTransaction(
        blockNumber,
        transactionId
      );
    }
  ...
  #getNewTransactions(): Transaction[] {
    ...
    const blockTransactionIds = transactions.map((transaction) =>
      transaction.getChannelHeader().getTxId()
    );
    const lastProcessedIndex = blockTransactionIds.indexOf(lastTransactionId);
    if (lastProcessedIndex < 0) {
      throw new Error(
        `Checkpoint transaction ID ${lastTransactionId} not found in block ${this.#block.getNumber()} containing transactions: ${blockTransactionIds.join(
          ', '
        )}`
      );
    }
...
function simulateFailureIfRequired(): void {
  if (
    ...
    transactionCount++ >= simulatedFailureCount
  ) {
    transactionCount = 0;
