...
const CONTROLLER_DID = 'did:web:gateway.example.org:VATES-B00000000:cds-EU:v1:research:employee:nuria:legal-representative';
const EXISTING_CONTROLLER_DID = 'did:web:gateway.example.org:VATES-B00000000:cds-EU:v1:research:employee:cto:legal-representative';
...
describe('persistExistingTenantControllerBinding', () => {
  it('adds an ICA-approved single-key Pontus-X controller without replacing the existing controller', async () => {
    ...
    await persistExistingTenantControllerBinding({
      ...
      controller: {
        did: CONTROLLER_DID,
        sameAs: 'nuriasala@unid.es',
        publicKeyJwk: publicKey('legacy-pontus-x', 'ES256K'),
      },
    ...
    expect(employee.indexed.attributes).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({ name: 'org.schema.Person.hasOccupation.identifier.value', value: 'RESPRSN' }),
