...
- reduce confusion between runtime, deployment, crypto, and storage settings,
...
- With `hashicorp-transit`, use the Transit key name.

`KMS_RUNTIME_KEK_ID`
- Stable service/environment identifier included in authenticated KMS context.

`KMS_RUNTIME_KEK_CIPHERTEXT`
- Provider ciphertext containing the 32-byte runtime KEK encrypted by the
  selected external root. Cloud providers commonly return Base64; Transit
  returns its versioned `vault:vN:` ciphertext form.
...
[24-fabric-authorization-ledger-binding.md](./24-fabric-authorization-ledger-binding.md).
...
`HLF_CERTIFICATE_Host1MSP`
`HLF_PRIVATE_KEY_Host1MSP`
`HLF_CONNECTION_PROFILE_Host1MSP`
`HLF_CONNECTION_PEM_Host1MSP`
`HLF_CONNECTION_PEER_Host1MSP`

`HLF_CERTIFICATE_Host2MSP`
`HLF_PRIVATE_KEY_Host2MSP`
`HLF_CONNECTION_PROFILE_Host2MSP`
`HLF_CONNECTION_PEM_Host2MSP`
`HLF_CONNECTION_PEER_Host2MSP`
...
## 22. GCP And Provider Credentials
