...
[[ "$(yq '.chaincodes | length' "${CHART}/ci/production-values.yaml")" == "10" ]] || {
  echo 'production example must enumerate all ten CCAAS runtimes' >&2
  exit 1
}
...
grep -q 'STORAGE_LAYOUT: "scoped-v2"' "${TMP_DIR}/production.yaml"
...
grep -q '^  name: host-gdc-host-orderer-tcp-bridge$' "${TMP_DIR}/production.yaml"
grep -q 'app.kubernetes.io/component: orderer-bridge' "${TMP_DIR}/production.yaml"
grep -q 'nc 192.0.2.10 7050' "${TMP_DIR}/production.yaml"
...
grep -q '/msp/intermediatecerts' "${TMP_DIR}/production.yaml"
