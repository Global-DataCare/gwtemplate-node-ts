...
if [[ -z "${AUTHORIZATION_JSON:-}" || -z "${CA_URL:-}" || -z "${CA_ADMIN_HOME:-}" \
  || -z "${CLIENT_ENROLLMENT_OUTPUT_FILE:-}" ]]; then
  echo 'Missing AUTHORIZATION_JSON, CA_URL, CA_ADMIN_HOME or CLIENT_ENROLLMENT_OUTPUT_FILE' >&2
  exit 1
fi
...
enrollment_id="${HOST_CLIENT_ENROLLMENT_ID:-host-gw-$(printf '%s' "${host_url}" | shasum -a 256 | cut -c1-17)}"
enrollment_secret="${HOST_CLIENT_ENROLLMENT_SECRET:-$(openssl rand -base64 36 | tr -d '=+/' | cut -c1-32)}"
