...
const outDir = path.resolve(ROOT, 'docs-v1', 'openapi-profiles');
...
function classifyPath(p) {
  ...
  // Every implemented non-compatibility route belongs to GW CORE. Product
  // extensions are documented and generated by their downstream repository.
  return 'core';
}
...
const CORE_FLOW_STEP_BY_PATH = {
  ...
  '/{tenantId}/cds-{jurisdiction}/v1/{sector}/individual/org.hl7.fhir.api/Patient/$match': '9.1',
};
...
function deriveSpec(spec, targetProfile) {
  ...
  for (const [p, item] of Object.entries(clone.paths || {})) {
    ...
    // Include core in both profiles; compat adds legacy aliases.
    const include =
      targetProfile === 'core'
        ? profile === 'core'
