...
IHE PDQm `POST Patient/$match` is the deliberate synchronous exception. It
returns a FHIR `Bundle` with `type=searchset` immediately after the same GW
transport authentication gates; it is never queued or polled. See
[the provider-resolution and Patient matching guide](../03-IDENTITY-AND-TRUST/03.B-PERSON-DISCOVERY-ACTION-ARCHITECTURE.md).
