...
## SEDIA deployment and publication boundaries

The deployed solution combines components with different responsibilities; it
does not treat Fabric and Pontus-X as interchangeable networks:

| Component | Current responsibility |
| --- | --- |
| Organization controller | Decides which products, datasets or digital-twin services the organization publishes. |
| External portal/BFF | Own the user-facing Pontus-X publication flow and the Pontus-X operational credentials. This responsibility is outside GW CORE and dataspace ICA. |
| Pontus-X | External federated catalog in which the controller publishes and consumers discover products and service offerings. |
| GW CORE | Provides authenticated/authorized APIs, FHIR searches and access to the hosted clinical or digital-twin data plane. |
| Dataspace ICA | Verifies organizations and representatives, issues or records trust evidence, and exposes technical trust/discovery information. It is not the primary Pontus-X product publisher. |
| Hyperledger Fabric | Permissioned ledger used for governed state, credential status and evidence/audit anchoring. It is not the Pontus-X product catalog. |

GW and ICA expose technical host, tenant, DID, DSP and DCAT discovery surfaces.
Those surfaces identify operators, participants, endpoints and capabilities;
they must not be described as the controller-facing Pontus-X marketplace
publication flow.

The reproducible local host deliverable covers GW CORE, dataspace ICA and the
permissioned Fabric runtime described by that deliverable. Pontus-X and the
the external portal/BFF are external and are not reproduced, simulated
or packaged by that local deliverable.
...
The identity model combines resolvable `did:web` identifiers, semantic URNs,
Verifiable Credentials and Hyperledger Fabric-backed governed state. Pontus-X
publication remains a separate portal/BFF-owned catalog concern.
...
Authorization identifiers use the shared CDS grammar and builders. Application
code must not assemble them from literals.

-   **Example for an Organization:**
    `urn:cds-es:v1:organization:tax:ES-B00112233`
-   **Example for an Employee (hierarchical):**
    `urn:cds-es:v1:organization:tax:ES-B00112233:member:<multibase-member-id>:ISCO-08|4110`

The member component is derived from protected input by the shared builder; an
email address must never appear in the identifier.
...
Fabric channel names are derived from sector + jurisdiction. In the current backend routing this is implemented as a simplified scheme (e.g., `health-care-eu`, `insurance-global`) and may later evolve into fully-qualified names including environment/version (see `docs-v1/04-DEEP-DIVES/04.D-DATASPACE-PUBLICATION-ATTESTATION.md`).
...
*   **Atomic Transactions (`_transaction`, `_seal`):** Result in a `transaction-response` bundle and are treated as certification/ledger-side operations; they do not implicitly update the individual's index.
*   **Independent Batch Operations (`_batch`, `_verify`):** Result in a `batch-response` bundle.

## 7. The `BundleEntry` Contract

*   The `claims` for business logic **MUST** be located in `entry.resource.meta.claims`.
...
This pattern is fundamental to the system's testability. For detailed
instructions on tests involving dependency injection, see the
[testing patterns guide](01.F-TESTING-PATTERNS.md).
