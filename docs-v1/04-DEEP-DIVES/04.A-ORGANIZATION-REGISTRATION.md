...
4.  **Message Decoding:** The backend's security layer decodes and verifies the message, resulting in a plaintext `IDecodedDidcommPayload` object. This object is the `requestPayload`.
...
7.  **Response Payload Creation:** The manager constructs and returns a complete `IDecodedDidcommPayload` object, which is a JARM-compliant payload containing the results of the operation in a `Bundle`.
8.  **Response Securing:** A `Worker` process takes the `IDecodedDidcommPayload`, passes it to the `IKmsService` to create a `secureEnvelope` (JWE/JWS).
...
### 3.3 Output (`IDecodedDidcommPayload`)

The entire response from the manager is a JARM-compliant `IDecodedDidcommPayload` object. The `body` of this object is a `Bundle`.
...
*   **Language:** TypeScript
*   **Responsibilities:** Follow the component responsibilities in the [architecture overview](../01-OVERVIEW-AND-GUIDES/01.A-ARCHITECTURE-OVERVIEW.md).
*   **Data Structures:** Adhere to the data structures defined in this document. The output of the `OrganizationManager` MUST be a `Promise<IDecodedDidcommPayload>`.
*   **Error Handling:** Implement robust error handling using the custom `ManagerError`.

**Example Prompt for AI:**

"Create a TypeScript class `OrganizationManager`. Its `register` method must accept a `JobRequest` object and return a `Promise<IDecodedDidcommPayload>`. The method should validate the input data from `job.input.body`, separate claims into resources, generate identifiers, and build the final `IDecodedDidcommPayload` as defined in the 'Organization Registration Process Documentation'."
