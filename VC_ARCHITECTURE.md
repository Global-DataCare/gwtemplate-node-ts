...
-   **Example:** `urn:antifraud:test-network:es:v1:health-care`
...
-   **Organization Identifier:** The primary identifier for a tenant.
    -   **Resulting Identifier:** `urn:antifraud:test-network:es:v1:health-care:entity:tax:B0011223344`

-   **The Connection Pattern (For Individuals):** An anchor for all individual-related interactions, acting as a stable, opaque proxy.
    -   **Resulting Identifier:** `urn:antifraud:test-network:es:v1:health-care:entity:alternatename:acme:connection:uuid:<uuid-v4>`

-   **Related Person Identifier (e.g., a parent of a child patient):**
    -   **Base:** Connection URN
    -   **Attribute Segment 3:** `related:ppn:es:A1234567B` (Identifies the related person, e.g., by passport number)
    -   **Attribute Segment 4:** `role:relationship:PRN` (Defines the relationship, e.g., Parent)
    -   **Resulting Identifier:** `...:connection:uuid:<uuid>:related:ppn:es:A1234567B:role:relationship:PRN`
...
-   **Professional Access Identifier (e.g., a doctor):**
    ...
    -   **Resulting Identifier:** `...:connection:uuid:<uuid>:employee:email:doctor@hospital.example.com:role:isco-08:2211`
...
1.  A preliminary VC object is created, containing all properties **except for the `id` and the `proof`**.
2.  This preliminary object is canonicalized to a stable string using the JCS (JSON Canonicalization Scheme, RFC 8785) standard.
3.  The SHA3-256 hash of the canonical string's byte representation is computed.
4.  This raw hash digest is then encoded using the **Multihash** format, which prepends the digest with a code identifying the hash algorithm (e.g., `sha3-256`).
5.  The resulting Multihash byte array is then encoded into a string using the **Multibase** format (e.g., `base58btc`, prefixed with `z`).
