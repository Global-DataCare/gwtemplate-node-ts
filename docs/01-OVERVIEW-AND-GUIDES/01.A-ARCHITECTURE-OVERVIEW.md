...
> ### Golden Rule: `didDocument.id` is the Resolvable `did:web`, `alsoKnownAs` contains the Canonical URN
>
...
>
> The canonical, persistent, and semantic identifier (the URN, e.g., `urn:unid:...`) is stored in the `alsoKnownAs` array. This provides a stable, location-independent identifier while maintaining full compatibility with URL-based DID resolution mechanisms. The `iss` claim in a JWS should always be a resolvable `did:web`.
...
-   **Breakdown:**
    -   `tenantId`: The `alternateName` of the entity responsible for the operation (e.g., `host`, `acme`).
