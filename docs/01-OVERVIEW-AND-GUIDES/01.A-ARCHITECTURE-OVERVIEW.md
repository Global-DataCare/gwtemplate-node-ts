...
| `globaldatacare.es` portal and BFF | Own the user-facing Pontus-X publication flow and the Pontus-X operational credentials. This responsibility is outside GW CORE and dataspace ICA. |
...
`globaldatacare.es` portal/BFF are external and are not reproduced, simulated
