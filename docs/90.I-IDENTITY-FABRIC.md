...
### CES mapping (Gaia-X)
Gaia-X introduces a Credential Event Service (CES) because there is no shared ledger. Within the GDC permissioned Fabric domain, ICA makes the decision (issue/revoke/suspend) and Hyperledger Fabric is the internal shared source of truth. This internal design does not replace CES for external Gaia-X/Pontus-X interoperability and must not be presented as a general CES equivalence. Within the GDC Fabric domain:
- Ledger state is the authoritative internal status feed.
...
This keeps `network` as a logical routing layer for the configured internal
ledger implementations, currently `mem`, `fabric` or their composed internal
mode. Pontus-X product publication is not a credential-ledger provider in GW;
it belongs to the organization controller through the portal/BFF.
...
### TODO for v1 lifecycle alignment
- `disable` in GW is a local suspension signal, not a credential revocation and not an implicit license release.
- A suspended employee/individual/tenant must remain auditable and reactivatable.
- License seats linked to suspended identities should be treated as suspended/reserved unless ICA or ledger policy explicitly returns them to an available pool.
- The authoritative source for VC suspension/revocation remains ICA plus the ledger `credentialStatus` contract.
- The host/portal should expose a canonical `/_deactivate` flow so ICA-backed suspension or revocation events can be submitted and audited without purging local history.
