...
Canonical reminder for document cases:

- teach `Bundle.type=document` with `Composition` first
- then use `Communication` as the DIDComm/plain wrapper or read path
- keep backend search examples separate and FHIR-parameter based
