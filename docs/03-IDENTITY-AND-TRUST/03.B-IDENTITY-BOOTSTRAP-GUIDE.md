...
*   Verifying one end-user `id_token` during onboarding does not eliminate the later DCR step:
    *   onboarding proves who the human controller is in the current session
    *   DCR registers which technical client/device/BFF is allowed to operate afterwards
*   If a legacy compatibility `Organization/_activate` flow is used and GW must publish the initial controller person DID in that step, the controller public material must arrive explicitly, for example via:
    *   `controller.did`
    *   `controller.sameAs`
...
sequenceDiagram
    ...
    participant GatewayA as Gateway API (Org A)
    participant Reconciler as Root-authorized Fabric Reconciler
    participant Fabric_CA as Fabric CA
    ...
    GatewayA->>GatewayA: Verifies proofs & obtains Host VC

    Note over GatewayA, Reconciler: Governed Membership
    GatewayA->>+Reconciler: 1. Host VC + Root governance decision
    Reconciler->>Reconciler: 2. Verifies issuer, MSP, peer and channels
    Reconciler->>+Fabric_CA: 3. Registers bounded MSP/TLS identities
    Fabric_CA-->>-Reconciler: 4. One-time enrollment material

    Note over GatewayA, GatewayB: Secure Secret Brokering
    Reconciler->>+GatewayB: 5. Encrypts and delivers enrollment material
    
    Note over GatewayB, Fabric_CA: Local Key + CSR Generation
    GatewayB->>+Fabric_CA: 6. Runs fabric-ca-client enroll locally for MSP/TLS
    Fabric_CA-->>-GatewayB: 7. Issues the X.509 certificates

    Note over GatewayA, GatewayB: Final Linkage
    Reconciler->>Reconciler: 8. Joins authorized channels and reconciles chaincodes
```
...
5.  `Active`: The onboarding is complete, and the tenant is operational on the host.
...
### Operational Route Sequence After Activation

The activation flow is not the same thing as service publication. After the
tenant becomes operational, the current route order is:
...
3. `POST /{tenantId}/cds-{jurisdiction}/v1/{sector}/identity/openid/Token/_exchange`
4. `POST /{tenantId}/cds-{jurisdiction}/v1/{sector}/identity/openid/Token/_exchange-response`
5. `POST /{tenantId}/cds-{jurisdiction}/v1/{sector}/identity/openid/Device/_dcr`
6. `POST /{tenantId}/cds-{jurisdiction}/v1/{sector}/identity/openid/Device/_dcr-response`

Semantics:

- `/_activate` binds legal/business onboarding to tenant operational metadata.
- `Token/_exchange` issues the initial technical access token for the app or
  device.
- `Device/_dcr` binds the technical client keys for OAuth/DIDComm runtime use.
- DCR does not by itself publish a controller person signing identity.
- The controller/person identity layer and the app/device/BFF runtime layer are
  intentionally separate:
  - controller binding / VC / `hasCredential.material` answers who is
    authorizing
  - `Token/_exchange` + `Device/_dcr` answers which technical client is
    operating

### Where CSR And Publication Fit

The hosting-operator certificate flow is downstream from activation, but it
currently applies to the hosting operator, not to each tenant:

1. The hosting operator first passes the hosting-validation flow.
2. The adhesion is formalized in a signed PDF.
3. The hosting operator receives the Fabric operational bootstrap/license.
4. The hosting operator generates or loads its key pair locally.
5. GW/controller performs operational Fabric enrollment.
6. GW stores and exposes the returned public certificate chain for the host.
7. dataspace ICA can later exchange `signed PDF + Fabric operational
   license/bootstrap evidence` for the Host VC.
8. GW publishes the tenant only after it is operational and its
   `org.schema.Service.serviceType` denotes an externally discoverable provider
   capability.

So the full runtime chain is:

- hosting validation
- signed adhesion PDF
- Fabric operational bootstrap/license
- tenant activation in GW
- technical exchange and DCR in GW
- host-only operational Fabric enrollment through GW
- Host VC issuance in dataspace ICA
...
4.  **Network Identity:** Verify the certificate's chain of trust for the operational Fabric certificate and, separately, verify the Host VC issued by dataspace ICA.
