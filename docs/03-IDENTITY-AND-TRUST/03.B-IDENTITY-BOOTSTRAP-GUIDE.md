...
The ICA-backed CSR flow is downstream from activation, but it currently applies
to the hosting operator, not to each tenant:

1. The hosting operator generates or loads its key pair locally.
2. GW receives the CSR and onboarding evidence.
3. GW calls ICA internally to enroll/sign the CSR.
4. GW stores and exposes the returned public certificate chain for the host.
5. GW publishes the tenant only after it is operational and its
   `org.schema.Service.serviceType` denotes an externally discoverable provider
   capability.

So the full runtime chain is:

- hosting operator onboarding in GW/ICA
- tenant activation in GW
- technical exchange and DCR in GW
- host-only ICA CSR enrollment through GW
