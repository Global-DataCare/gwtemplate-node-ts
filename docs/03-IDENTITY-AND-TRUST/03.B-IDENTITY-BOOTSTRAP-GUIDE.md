...
The ICA-backed CSR flow is downstream from activation and adjacent to exchange
and DCR, but it solves a different problem:

1. A host operator or tenant generates its key pair locally.
2. GW receives the CSR and onboarding evidence.
3. GW calls ICA internally to enroll/sign the CSR.
4. GW stores or exposes the returned public certificate chain.
...
- optional ICA CSR enrollment through GW
