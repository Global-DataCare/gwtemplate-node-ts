...
*   **Example for a Tenant:**
    *   `didDocument.id`: `urn:antifraud:test-network:cds-es:v1:health-care:entity:TAX:B123...`
...
*   **Example for a Tenant:**
    *   `didDocument.alsoKnownAs`:
        *   `'did:web:host.example.com:acme:cds-<jurisdiction:>v1:<sector>'` (The hosted endpoint on the infrastructure provider).
        *   `'did:web:acme.org'` (The tenant's own corporate domain).
