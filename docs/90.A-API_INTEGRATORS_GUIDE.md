...
Read this guide as secondary material.

Before editing payloads, OpenAPI examples, or SDK integration snippets, read:
...
Canonical documentation rules now live in:

- `docs/API_CORE_INTEGRATION.md`
- `docs/00-READING_ORDER.md`

Canonical tests for the document story:

- `src/__tests__/unit/api-examples/communication.examples.test.ts`
- `src/__tests__/integration/composition.bundle-search.api.test.ts`

When this file overlaps with the canonical docs, prefer the canonical docs for:

- host/operator routing semantics
- controller binding semantics
- subject/professional/controller variable names
- SMART scope teaching order
- outbox-vs-network submission explanations
- communication layering and DIDComm versus batch-body semantics
- `Subject/$summary` target semantics versus compatibility `_search` routes

Memory aid for this transitional guide:
...
- `resource.resourceType` is only the outer resource shape or projection.
- `resource.meta.claims` is the canonical project-specific claims contract. It is not part of base FHIR.
- `Communication.contentdata` is one of those claims when a communication carries embedded payload data.

Teaching rule:

- New client examples should author canonical `resource.meta.claims` first.
- Internal models such as `CommMsgExtended` may appear in implementation notes, but they are not the primary outbound client contract.
- If a versioned FHIR payload is submitted, backend extraction/normalization derives the canonical claims from that payload.
...
- This file still contains some compatibility and legacy-oriented `_search` examples.
- Do not treat every `_search` example in this file as the preferred teaching model for new summary-oriented integrations.
- When the intent is patient or subject summary retrieval, prefer the canonical explanation in `docs/API_CORE_INTEGRATION.md` and the shared IPS outbox docs.
...
One canonical tenant may split operational capabilities across runtimes
without changing its VAT-backed `tenantId`. The deployment-owned
`TENANT_SERVICE_ROUTES_JSON` map overrides the operational base URL by GW
section for the named tenant while the public DID and well-known documents
remain canonical. For example:

```json
{
  "participant-tenant-id": {
    "default": "https://institutional-operator.example",
    "entity": "https://institutional-operator.example",
    "individual": "https://individual-operator.example",
    "digitaltwin": "https://individual-operator.example"
  }
}
```

This is routing metadata, not a second tenant registration. Each URL is
required to use HTTPS. The optional `default` route owns well-known, identity,
messaging, DSP/DCP and other shared services, and is also the fallback for
sections without a more specific route. Without `default`, sections not
present in the map continue to use `org.schema.Service.url` (or the normal
hosted-gateway fallback).

## ICA-Signed X.509 for the Hosting Operator

This platform currently requires the **hosting operator** to hold an
**ICA-signed X.509 certificate** while **private keys stay local** in the
gateway/KMS environment.

The ICA signs a CSR, and the signed certificate is then attached to the host's
public metadata (`jwks.json` / `x509.der`).

**Goals**
- Private key never leaves the hosting-operator environment.
- ICA only signs a CSR after host verification.
- X.509 chain is publishable and auditable.

**High-level steps**
1. **Key pair & CSR (host/gwtemplate):**  
   The hosting operator generates a key pair locally and creates a CSR (PKCS#10) with:
   - `CN = alternateName` (service name, e.g., “Participant - Regional Operator (Test)”)
   - `SANs = hosting operator domains/subdomains`
   - `OID 2.5.4.97 (organizationIdentifier) = TAX<country>-<taxId>`
2. **Signed request to host:**  
   The hosting operator submits the CSR plus its `schema.org Organization`
   JSON, signed with the private key to prove possession.
3. **Host validation:**  
   The host verifies schema.org fields, tax/country, CSR signature, and
...
- Hosted tenants receive VCs through the ICA-backed activation/verification
  flow.
- Hosted tenants do not currently receive individual Fabric certificates.
...
The Gateway's architecture distinctly separates public discovery from secure data exchange.

| Feature           | Public Discovery (`/.well-known`)                         | Secure Data Exchange (canonical example: `/Communication/_batch`) |
| ----------------- | --------------------------------------------------------- | ----------------------------------------------------------------- |
| **Method**        | `GET`                                                     | `POST`                                                            |
| **Synchronicity** | **Synchronous**                                           | **Asynchronous**                                                  |
| **Security**      | Public, Unprotected (Served over HTTPS)                   | End-to-End Encrypted (JWE/DIDComm)                                |
| **Purpose**       | Discovering public identity, keys, and capabilities.      | Securely exchanging confidential operational payloads.            |

Rule for the current core profile:

- Index operations are exchanged through `Communication`.
- `Communication/_batch` is the canonical secure exchange envelope for index-oriented flows.
- A batch may carry one or more `Communication` entries.
- `Consent` remains an important governed resource, but it should not be taught as the canonical envelope for index exchange.
...
2.  The server immediately returns an `HTTP 202 Accepted` response acknowledging the async job.
3.  In tests/demo, returning the `thid` in the response can help debugging.
4.  In production, that is not strictly necessary when the client generated the `thid` itself.
5.  The client polls a separate endpoint using that same `thid` until the final, secure JARM response is available.
6.  The polling endpoint provides the final JWE/DIDComm message for the client to decrypt.
...
- `taxTenantId`: canonical legal-organization identifier used as both the tenant path id and the legal tax/registration id in current examples (default: `acme-id`)
- `jurisdiction`: `ES`
- `sector`: `health-care`
- `network type` (host sector): `test`
- `portal namespace`: `globaldatacare.es`
- `individualUuid`: private UUID hashed as its canonical 16 bytes with SHA3-384
- `individualProviderDid`: exact provider DID returned in `Offer.offeredBy`
- `individualDid`: hosted DID derived from those two values; the concrete demo
  value is
  `did:web:host.example.org:health-care:organization:taxid:ES-B00112233:individual:UUID:zG9H82pae9SCXvec3D4YKqhX8bj8F1mRgzxMEdwXXonT7BWsvsUiP2u52sWQTeESpoMee`
- `individualControllerDid`: member DID derived from the controller email and
  role. The concrete demo value is
  `did:web:host.example.org:health-care:organization:taxid:ES-B00112233:individual:UUID:zG9H82pae9SCXvec3D4YKqhX8bj8F1mRgzxMEdwXXonT7BWsvsUiP2u52sWQTeESpoMee:member:zG9DrMLpQW8eoCc9Ay9AFxuMGiswgJePpbUMz9svJCZ8tKjUd4xoExgCPA5jmHc6hPATJ:RESPRSN`
- `physicianEmail`: `doctor1@acme.org`
- `physicianRole`: `ISCO-08|2211`
- `sectionsAllowed`: `LOINC|48765-2`
- `physicianOrg`: `did:web:globaldatacare.es:<sector>:organization:taxid:<taxTenantId>`

What is auto-applied at request execution time:
- Path params: `tenantId`, `jurisdiction`, `sector`
- `tenantId` and `Organization.identifier.value` are forced to the same `taxTenantId` value in the Swagger helper
...
- Known claim overrides:
  - tenant alternate name (family/individual examples)
  - canonical legal identifier value
  - physician email / role
  - consent/composition allowed section

For an individual-controller DCR, `actor_did` and `profile_did` are both the
complete member DID above. The protected licence binds its parent individual
DID, the verified email or phone, and the full role token
`http://terminology.hl7.org/CodeSystem/v3-RoleCode|RESPRSN`; only `RESPRSN` is
serialized in the DID. `clinical_creator_binding` uses separate stable UUID
URNs for the natural actor and its RelatedPerson assignment, the individual's
`urn:uuid` as owner, and the same full role token:

```json
{
  "kind": "individual-member",
  "actorIdentifier": "urn:uuid:8c18ec13-e908-4b7a-91bb-134bb3a229e1",
  "authorIdentifier": "urn:uuid:033ceb35-2528-402e-8385-f22e12f57805",
  "ownerIdentifier": "urn:uuid:a87e5b15-aea4-4475-9c7c-40aa88354b6f",
  "role": "http://terminology.hl7.org/CodeSystem/v3-RoleCode|RESPRSN"
}
```

That individual-controller `ownerIdentifier` is the licensed individual's
UUID URN; it is not an organization identifier. In a professional employee
binding, by contrast, `ownerIdentifier` is the host-independent legal
organization author, for example
`urn:cds-es:v1:organization:tax:ES-B00112233`; `authorIdentifier` remains the
exact PractitionerRole `urn:uuid` and `actor_did` remains the operational
employee `did:web`.

The activation code is returned by the successful Order response. The
canonical response claim location is `data[0].resource.meta.claims`; consumers
may read `data[0].meta.claims` only as a compatibility fallback for older GW
responses.
  - permission-grantee / actor organization DID
  - individual subject identifiers and common DID placeholders
  - service category
...
- `individualDid` is intended for step-by-step individual/consent/composition flows
- `physicianOrg` is intended for permission grants and defaults to the portal DID style
  `did:web:globaldatacare.es:<sector>:organization:taxid:<taxTenantId>`
...
This section details the secure workflow for onboarding a new organization. The current canonical flow starts with host-side `Organization/_transaction`, which forwards legal evidence and business binding material to ICA `_verify` and returns the host continuation contract without requiring `Organization/_activate` as a normal next step. The transport profile remains based on OpenID Connect (OIDC), Dynamic Client Registration (DCR), DIDComm plaintext in non-production, and the Financial-grade API (FAPI) Security Profile in production.

There are three host onboarding modes:

- Canonical first step: host-side organization verification transaction via `Organization/_transaction`, where GW CORE forwards the signed PDF evidence, controller business binding key, optional organization signing key, and representative payload to ICA `_verify`.
- Legacy compatibility route: host-side organization activation via `Organization/_activate`, for callers that already start from an ICA-issued proof and still use the older proof-consumption contract.
...
- Canonical ICA verification returns three independent credentials:
  `OrganizationCredential`, `LegalRepresentativeCredential` and
  `ServiceControllerCredential`.
- `LegalRepresentativeCredential.hasOccupation.occupationalCategory` carries
  the ISCO occupation, default `ISCO-08|1120`.
- `ServiceControllerCredential.owner.additionalType` carries bare `RESPRSN`.
  Its independent professional occupation is
  `owner.hasOccupation.occupationalCategory`, default `ISCO-08|1330`.
- Flattened `_transaction` claims belong in
  `body.data[].resource.meta.claims`; entry-level `meta.claims` is accepted only
  as deprecated compatibility input.
- Test Network sends `OrganizationTestNetworkCredential` only through
  `body.data[].resource.organizationTestNetworkCredential` and exactly three
  domain credentials through `body.data[].resource.testNetworkCredentials`.
  No authorization-named credential or transport alias exists.
...
- The legal-organization activation must also declare the provider capabilities that will be published after activation in `org.schema.Service.serviceType`.
...
- Representative/controller proof is intentionally two-dimensional:
  - `credentialSubject.sameAs`
    public identity continuity of the representative
  - `credentialSubject.hasCredential.material`
    signing-key continuity of the controller binding
  - production-grade ICA-issued representative VCs should ideally carry both
...
For an existing tenant, `Organization/_issue` returns all ICA-issued
credentials in the response entry `vc[]`. A consuming BFF must upsert the
`OrganizationCredential`, `LegalRepresentativeCredential` and
`ServiceControllerCredential` into its own organization, representative and
controller records respectively. GW CORE does not own that portal database.
GW CORE consumes the `ServiceControllerCredential` to append or refresh the
technical controller employee and tenant DID controller reference; it never
promotes a representative carrying only `ISCO-08|1120` to controller.
...
For demonstration purposes, `Content-Type: application/didcomm-plain+json` (or `application/json`) is permitted. However, in production, all messages must be secured using the FAPI Security Profile (based on JAR and JARM), where `Content-Type: application/x-www-form-urlencoded` is used to submit and receive encrypted messages via the `request` and `response` form parameters.
...
- New ICA-first onboarding should prefer `Organization/_transaction`. `Organization/_activate` is a legacy compatibility route and is not required when the onboarding starts with `_transaction`.
...
**`curl` Example:**
<!-- sync-example: ORGANIZATION_REGISTRATION_REQUEST -->
```bash
    ...
    "data": [
      {
        ...
        "resource": {
          "meta": {
            ...
            "org.schema.Organization.identifier.value": "acme-id",
            ...
            "org.schema.Service.serviceType": "organization/Composition.cruds,organization/ResearchSubject.rs",
...
Testing note:
- `org.schema.Organization.identifier.value` (Tax ID in current ES examples) participates in tenant collection naming and should be unique per organization registration.
- For demo flows in `/api-docs`, `taxTenantId` is the single canonical value for both `tenantId` and `Organization.identifier.value` in legal-organization examples.
- In `v1.x`, legal-organization examples omit `Organization.alternateName`; GW CORE derives its internal compatibility alias from `taxId` when needed. Public `alternateName` examples are reserved for individual/family onboarding.
...
### 6.2. Legacy Compatibility Route: Activate a New Tenant from ICA Proof

This is a legacy compatibility route for organizations that already start from ICA-issued proof and still use the older activation contract.
...
- `Organization.hasCredential.material`
  public cryptographic material of the organization when that binding is carried in the organization credential
- `Person.hasCredential.material`
  public cryptographic material of the controller/person when that binding is carried in the representative credential
- if ICA also issues a `SoftwareApplication` VC for the portal/backend:
  - `SoftwareApplication.material` is the public cryptographic material of that software application
  - in practice this is typically the communication signing key id bound during the ICA registration step
  - when represented as a JWK thumbprint identifier, RFC 7638 defines the canonical thumbprint calculation over the public signing / verification JWK, and RFC 9278 defines the canonical URN form `urn:ietf:params:oauth:jwk-thumbprint:sha-256:<base64url>`
  - the controller signature belongs to that earlier ICA registration step, not to every later operational app-service proof
...
- `Organization/_activate` is legacy compatibility. It is not a required continuation step after a successful `Organization/_transaction`.
...
**`curl` Example:**
<!-- sync-example: ORGANIZATION_ORDER_REQUEST -->
```bash
# -X POST: Specifies the HTTP POST method.
# --header: Provides necessary HTTP headers for the request.
#   - Authorization: Contains the OIDC id_token of the legal representative.
...
**Invoice format note:** The current order response keeps the flat compatibility claims (`Order.partOfInvoice`,
`Order.paymentUrl`) and also returns an embedded invoice `Bundle` with:
- one FHIR `Invoice`
- one PDF `DocumentReference`
- one structured `DocumentReference` carrying JSON/XML invoice payloads
Spain’s **VeriFactu** is a national anti-fraud system (AEAT) and is **not** based on UBL; it still belongs to a
future country-specific billing adapter layer.

### 6.4. Host CSR Enrollment Status

Host CSR enrollment is not part of the tenant messaging journey.

- The current hosted-tenant onboarding flow ends with tenant activation on the
  host.
- The hosting operator may perform its own operational Fabric enrollment
  separately for its operator certificate lifecycle, after the hosting
  validation/adhesion flow and the corresponding Fabric bootstrap/license.
- That host certificate flow must not be treated as a tenant-facing onboarding
  message contract.
...
For a professional-seat purchase, use the tenant `Offer/_create` capability and
send only the requested quantity and professional user class through the
high-level SDK. GW authors the identifier, serials, price, currency and allowed
payment methods, protects that Offer under host custody, and the controller
confirms the exact returned identifier through host `Order/_batch`.

After DCR, controller key custody always remains in the logical tenant vault.
A controller DID may contain the official organization identifier rather than
the tenant alternate name; GW resolves it through the canonical tenant registry
before calling Firestore, PostgreSQL, memory or another repository adapter. A
trusted OIDC account `sub` is provider-local and need not equal the actor DID:
authorization requires either the exact DID subject or the same verified
email/telephone stable identifier encoded by the canonical professional DID.
This resolution never shares HMAC secrets, private keys or tenant data with a
portal or with the host configuration vault.

The shared path segment is `businessSectorOrNetworkKind`. On tenant routes it
contains the business sector; on host commercial routes it contains the exact
deployment `networkKind`. The stages `test` (in-memory, without blockchain),
`local-network`, `test-network`, and `network` are distinct and ordered, not
aliases. A client must propagate the configured kind exactly.

The host authors the onboarding Offer, so its CDS URN uses the host's legal
jurisdiction. The applicant country remains authoritative for the hosted
organization DID and credentials; route coverage is a separate dimension. A
pending Offer remains retryable until mandatory ledger registration succeeds,
and the active tenant retains its Offer index for idempotent Order replay.
...
**Current implementation note:** The codebase currently implements **seat gating** using the tenant vault section **`device-licenses`** (a pool of `DeviceLicense` documents stored under the tenant's vault/collection via the vault repository abstraction). In the current portal-managed mode, the portal/BFF closes payment outside GW CORE, then submits the paid `Order` confirmation back to GW so the accepted offer can emit `device-licenses`. The old GW-generated payment-link flow remains only as one compatibility mode for confidential-app scenarios or direct tenant-managed payment UX.

**Portal-managed payment rule:** For the current non-confidential portal journey, treat the host `Order/_batch` as the business confirmation sent **after** the BFF has already received payment confirmation from Stripe or another provider. The BFF must include `Order.paymentMethod` plus `Order.partOfInvoice` or `Order.paymentUrl`, including for zero-price Test Network offers, and GW CORE verifies that proof before emitting seats. Test deployments may use the configured mock payment verifier; they still persist the Order and invoice projection.
...
For demo purposes, use `Content-Type: application/didcomm-plain+json` in the HTTP Header. In production the requisite is to enable secure DIDComm messages using the FAPI Security Profile (based on JAR and JARM), where `Content-Type: application/x-www-form-urlencoded` enables the `request` and `response` form parameters to submit and receive the encrypted messages.
...
1. A valid **user identity** token from one of the OIDC providers trusted by this GW deployment, and
2. A pre-purchased license **activation code** (`subject_token`).

The deployed GW may trust more than one provider directly. Set
`AUTH_TOKEN_VERIFIER=trusted-oidc` and configure exact `iss` and `aud` pairs
in `OIDC_TRUSTED_PROVIDERS_JSON`, using the JWT claim names directly. The
legacy configuration names `issuer` and `audience` remain accepted only as
non-conflicting aliases. The GW obtains `jwks_uri` through the
issuer's standard OpenID configuration document; an unverified `iss` is used
only to select the configured verifier, never as proof of identity.

```json
[
  { "issuer": "globaldatacare.es", "audience": "globaldatacare.es" },
  { "issuer": "https://securetoken.google.com/unid-production", "audience": "unid-production" }
]
```

This list belongs to one deployed GW and therefore one network. It is not a
cross-network production/staging trust registry.

#### 7.1.1. (Optional) Federate external identity into Firebase

This compatibility step is required only when the current GW does not trust the external issuer directly.

- If the issuer/audience pair is in `OIDC_TRUSTED_PROVIDERS_JSON`, send that provider's `id_token` directly.
- Otherwise federate through Firebase and send the resulting Firebase `id_token`.

Firebase remains a supported identity broker, but it is not mandatory when the external OIDC provider is explicitly trusted by this GW.
...
`Token/_exchange` requires a seat activation credential (`subject_token`). Licenses live in the tenant vault section `device-licenses` and are created as `available` seats after purchase (for example, employee seats for professionals and individual seats for indexed members). The `Offer.serialNumber` list represents **seat IDs** (the pool), not activation credentials.

To invite a professional (or reserve a seat for a specific email + role), a tenant controller (or an IT employee with delegated permissions) issues an activation credential from that pool. That credential can activate distinct installations up to `maxDevices` (five by default). Reissuing rotates the credential for the same actor and seat; it does not allocate another seat.

**Seat policy:** the seat represents one user entitlement. Each distinct
`client_instance_id` creates an active binding. Repeating the same identifier
rotates/replaces that installation; a new identifier adds another binding until
the allowance is full. `Device/_revoke` releases one binding without releasing
the employee seat.
...
- Repeating an already accepted organization Order returns the same unconsumed
  first activation code. The replay must not rotate it, allocate another seat
  or require another commercial confirmation.
...
- **Hybrid SMART (with professional VP):** After the employee/family VC is issued, subsequent SMART requests
  include the VP so the index provider can verify professional evidence before issuance. During migration,
  sibling `body.vp_token` remains accepted while the canonical presentation moves to `client_assertion.vp`.
  Cross-custodian verification and the signed tenant introspection response
  carrying a compact JWT-VP with the unchanged professional VC are defined in
...
- **More than one controller:** Adding a legal representative appends that person's DID to the organization DID
  `controller` array; it never replaces the current controller. Every controller remains a separate Person/Employee
  record with its own DID document and public keys. DCR subsequently registers that person's device keys; it does not
  create or transfer the actor/controller key. Public JWK `kid` values and `Person.hasCredential.material` indexes use
  the RFC 9278 `urn:ietf:params:oauth:jwk-thumbprint:sha-256:<base64url>` form. The add/rotation request must be signed
  by one active controller already referenced by the organization DID; the new controller key cannot authorize itself.
  One actor public JWK is sufficient here. Do not include DCR/device keys merely to satisfy controller registration.
...
1. A valid **user identity** token from a provider trusted by this GW, and
2. A pre-purchased license **activation code** (`subject_token`).
...
- Header: `Authorization: Bearer <trusted_provider_id_token>`
- Header: `Content-Type: application/didcomm-plain+json`

Important:
- `Authorization` must carry a valid JWT identity token whose exact `iss` and `aud` match one configured provider.
...
#### 7.1.2.1. Employee wallet replacement after PIN loss

`POST /host/cds-{jurisdiction}/v1/{sector}/{tenantId}/identity/auth/_recover`
is a BFF-only employee recovery action. It requires a Firebase `id_token`
issued by a fresh email OTP ceremony (no more than five minutes old) and the
stored `client_instance_id`. GW verifies exact email ownership of that active
employee installation and rotates its activation credential so the SDK can
perform DCR replacement with newly generated keys.

The browser sends only the OTP through the authentication ceremony and enters
the new PIN twice in the portal. It never receives the replacement activation
credential, old seed or private keys. The old PIN is neither accepted nor
required; this is key replacement, not recovery of the old wallet seed.
...
-   **`body.code`**: The seat activation credential already validated during exchange.
...
schema.org `SoftwareApplication` claims for consistent indexing. In the GDC
profile, `SoftwareApplication.material` is the public cryptographic material of
the software application, typically the communication signing key id once ICA
binds it during registration.
...
- This `_search` route is an administrative listing endpoint.
- It is not the preferred teaching model for new summary-oriented clinical retrieval flows.
- For summary retrieval semantics, prefer the canonical `Subject/$summary` explanation in `docs/API_CORE_INTEGRATION.md`.
...
- The gateway accepts a signed client-authentication JWT in `client_assertion`.
- The canonical field names are:
  - `client_assertion`
  - `client_assertion_type`
- For compatibility, `client_assertion_type` may be sent either as:
  - `urn:ietf:params:oauth:client-assertion-type:jwt-bearer`
  - `private_key_jwt`
- This client-authentication layer is separate from the authorization proof layer:
  - professional / individual SMART access commonly still uses `vp_token`
  - inter-tenant research access may instead use an external `Bearer data access token`

**Proof requirement (SMART + Gaia-X Clearing House):**
- The SMART authorization request normally includes a verifiable presentation (VP) in the request object (JAR) or in
  the DIDComm payload when using the demo flow. This is the canonical proof path used to prove non-revocation via the
  Gaia-X Clearing House before issuing a SMART access token.
- For the inter-tenant `RESEARCH` use case only, the gateway also accepts one already-validated external
  `Bearer data access token` instead of `vp_token`, provided that the trusted issuer, provider tenant, consumer
  organization, purpose, and requested capability all match the request.
- The inter-tenant FHIR `Contract` VC gate applies to
  `organization/ResearchSubject.*` digital-twin capabilities. An individual
  `organization/Composition.*` self-read remains subject to VP verification
  and the individual's FHIR `Consent` rules, but does not require a research
  contract merely because the individual's public DID root differs from the
  operator DID serving the endpoint.
- Same-tenant Digital Twin access is not established by comparing DID hosts or
  aliases. GW extracts the VAT/tax tenant id, employee/member `z...` identifier
  and role from `sub`, supporting both its internal hosted DID and the public
  `did:web:<host>:<sector>:organization:(taxid|vatid):<VAT>:...` form. It then
  opens that tenant's employee registry and requires an active employee whose
  encrypted normalized email recomputes to the same
  `urn:multibase:z...` identifier and whose role matches. The already verified
  `EmployeeCredential` must bind the exact actor, role and `sameAs`. The clear
  email is never embedded in the DID or recovered from the hash.
...
-   `cty` (recommended): `"application/didcomm-plain+json"` (the signed payload is a DIDComm plaintext JSON object).
...
real SMART `/authorize` request, including PKCE parameters, client-authentication JWT fields, and `vp_token`.
The gateway accepts `client_assertion` in this demo-oriented flow even though it is not yet a full standalone
...
-   **`client_assertion`:** Signed client-authentication JWT proving possession of the registered device private key.
-   **`client_assertion_type`:** Usually the standard JWT-bearer URN; `private_key_jwt` is also accepted as a
    compatibility label.
-   **`vp_token`:** Verifiable Presentation (JWS or JSON-LD VP) validated by the Gaia-X Clearing House.
    For the inter-tenant `RESEARCH` variant, this proof may be replaced by one already-validated external
...
<!-- sync-example: SMART_TOKEN_REQUEST -->
```bash
...
**License gating (employee seats):** `Employee/_batch` with a create entry is the licensed interactive operation. If no employee seat is `available`, including when no pool exists yet, the final polled bundle entry is an Offer (`Employee-license-offer-v1.0`) containing `org.schema.Offer.*` claims instead of creating the employee. After `Order/_batch` materializes the seat, the caller retries creation and then performs `License/_issue`. A future governed bulk importer requires its own explicit operation contract; import semantics must not be encoded as private Person claims or inferred from this endpoint.
...
#### 8.2.0. Prerequisite: Provider discovery from operator DSP catalog
...
3. Resolve the advertised DSP entrypoint:
...
5. Optionally fetch dataset detail:
   - `GET /host/cds-{hostCoverageScope}/{version}/{hostNetwork}/dsp/catalog/datasets/{id}`

In GW CORE, `/dsp/catalog/request` is the current project-local route used
for DSP catalog requests. The DSP standard HTTPS binding path is
`/catalog/request`.
...
The catalog may also expose `dcat:service` entries pointing to the tenant's published service offerings:
- `/.well-known/service-offering-index.json`
- `/.well-known/service-offering-research.json`

Those offerings are derived from the activated `org.schema.Service.serviceType` claim:
- `indexing.*` capabilities publish the index offering
- `digitaltwin.*` capabilities publish the research offering
- those same capability families are what the SDK must declare during legal-organization `_activate`
...
**`curl` Example:**
<!-- sync-example: FAMILY_REGISTRATION_REQUEST -->
```bash
      ...
      "resource": {
        "meta": {
          "claims": {
            ...
            "Organization.identifier.value": "a87e5b15-aea4-4475-9c7c-40aa88354b6f",
            ...
            "Service.serviceType": "organization/Composition.cruds,organization/ResearchSubject.rs",
            "Service.termsOfService": "https://portal.example.org/files/legal-organization-signed-terms.pdf"
          }
        }
      }
    }]
...
<!-- sync-example: FAMILY_ORDER_REQUEST -->
```bash
...
**Normalization contract:** See [gdc-common-utils-ts/docs/normalization-spec.md](https://github.com/Global-DataCare/gdc-common-utils-ts/blob/main/docs/normalization-spec.md) for the canonical cross-repository rules.
...
**Next authorization task reference:** See [gdc-common-utils-ts/docs/consent-access-matrix-task.md](https://github.com/Global-DataCare/gdc-common-utils-ts/blob/main/docs/consent-access-matrix-task.md) for the follow-up cross-repository work on:
...
- `CommMsgExtended` is an internal or SDK-side helper model.
- It is not the preferred primary wire contract for a new client integration.
- New clients should build `Communication` entries whose canonical semantics live in `resource.meta.claims`.
- For embedded payload data, the canonical claim is `Communication.contentdata`.
- If a versioned FHIR resource is sent instead, backend extraction/normalization derives the stored claims.

Recommended mapping for frontend assistant flows:

1. Assistant produces one or more communication intents as canonical claims (including `Communication.contentdata` when embedding payload data).
...
Minimal document-index projection:

- A metadata-only document must opt in explicitly with a root `Bundle.meta.tag`
  coding whose system is
  `http://terminology.hl7.org/CodeSystem/v3-ObservationValue` and whose code is
  `SUBSETTED`.
- Only in that marked document does GW reduce tagged inner resources to
  `resourceType`, `id`, `fullUrl` linkage and the governed searchable claims
  derived from each `resource.meta.tag`; all other submitted inner fields are
  ignored.
- Without that root marker, inner `resource.meta.tag` values are ordinary FHIR
  metadata and the complete FHIR document import contract remains intact.

V2 direction note:

- This transitional guide still contains older direct `Composition/_batch` and
  `Bundle/_batch` examples later on.
- For the v2 teaching model, individual-index data should be exchanged through
  `Communication`.
- The canonical SDK/backend representation is still `resource.meta.claims`,
  including `Communication.contentdata` for embedded payload data.
- Versioned FHIR attachment fields are compatibility or projection shapes and
  should not be the first mental model taught to new integrators.
- `contentReference` remains valid for externally stored artifacts, but it
  should not be the only mental model taught for v2.

### DocumentReference compatibility ingestion (attachment-first)

For attachment-heavy compatibility cases, `DocumentReference` remains available as a lower-level wrapper. It is not the main teaching path for current individual document cases; the canonical story starts with `Bundle type="document"` and a first-entry `Composition`, then uses `Communication` for DIDComm/plain transport/readback.

The compatibility contract is **FHIR-based** (not schema.org) and **version-agnostic** at integration level:
...
1. **Phase 1 (compatibility only):** Create `DocumentReference` when the client cannot send the document bundle directly.
...
- Claims-first payloads go in `body.data[].resource.meta.claims` (`@context: org.hl7.fhir.api` + `DocumentReference.*` keys).
...
Canonical claims-first note:

- In the SDK/backend contract, the primary semantic source of truth is not the
  versioned FHIR attachment fields by themselves.
- The canonical stored/transported semantics live in `resource.meta.claims`.
- For current individual document cases, teach the document first: `Bundle`
  `type="document"` with `Composition` as the first entry.
- `Communication` is the transport/read wrapper around that attached document
  bundle in the DIDComm/plain path.
- If a versioned FHIR `Communication.payload[].contentAttachment.data` is used,
  backend extraction/normalization derives the canonical claims from it.
- `Communication.contentdata` remains a canonical claim for embedded payload
  data, but it is a secondary teaching path here.

Frontend/SDK may send the attached document bundle inline or by reference:
...
- Prefer the document bundle story first: `Bundle type=document` with
  `Composition` first, then `Communication` as the wrapper.
- Versioned `contentAttachment.data` examples should be presented as the
  frontend/BFF read shape around that document bundle.
- Treat direct resource-specific ingestion routes and raw plumbing as
  compatibility or lower-level runtime paths unless the example is explicitly
  about those routes.
...
- If user confirms extraction, build a `Bundle` `type="document"` with `Composition` first and `Composition.section` set to the target history section (for example `LOINC|8716-3` for Vital signs in healthcare).
...
- These audit `_search` endpoints are observability and target-architecture examples.
- They should not be used as the primary teaching example for clinical summary retrieval.
...
- The direct `Composition/_batch` flow below is a legacy or lower-level route
  formally deprecated and kept unchanged for existing integrations.
- `Composition/_batch-response` is deprecated with its submit route.
- It is not the preferred v2 teaching model for index exchange.
- In the v2 model, teach the document bundle first (`Bundle type="document"`
  with `Composition` first), then show `Communication` as the DIDComm/plain
  wrapper or read path.
- `Communication.contentAttachment.data` remains a compatibility or projection
  shape, not the main story.
...
**Deprecated:** new integrations use `Communication/_batch` through the SDK
`updateClinicalSummary(...)` or `updateSubjectSection(...)` facade. The example
below documents compatibility behavior only.
...
#### 9.A Internal pre-conversion adapter compatibility (digital twin)

This lower-level route is reserved for explicitly authorized pre-conversion
adapters that already received a GW-registered pseudonymous twin subject
through controlled internal orchestration. It is not the patient-portal flow.
A portal enables or disables the FHIR Consent and continues ingesting the IPS
through `individual/.../Communication/_batch`; GW owns pseudonymization and
projection.

An adapter must not submit an operational DID, derive a subject from source
identifiers or invent a UUID. It submits one Composition per source IPS
document/version with all applicable sections and uses only the registered
`urn:uuid` supplied by GW.
...
- Expected adapter output shape:
  ...
  - `body.data[]` (one Composition per IPS document/version)
    ...
    "data": [{
      ...
      "resource": {
        ...
        "meta": {
          "claims": {
            "@context": "org.hl7.fhir.api",
            "Composition.subject": "[GW_REGISTERED_TWIN_URN_UUID]",
            "Composition.section": "LOINC|26436-6,LOINC|30954-2",
            "Composition.type": "LOINC|60591-5",
        ...
        "contained": [{
          ...
          "meta": {
            "claims": {
              ...
              "DocumentReference.subject": "[GW_REGISTERED_TWIN_URN_UUID]",
...
Current scope note: the gateway persists Composition-level claims for digital
twin indexing. `contained[].meta.claims` are accepted for compatibility with
pre-conversion output, but `Encounter`/`Patient` persistence is not active yet
in this flow. There is no public portal operation for discovering the private
operational-subject mapping; adapters cannot use this compatibility route to
bypass the normal Consent plus individual Communication lifecycle.

---

#### 9.B Digital twin search by IPS section and textual claims

The public search contract for digital twin retrieval is now:

- `POST /{tenantId}/cds-{jurisdiction}/v1/{sector}/digitaltwin/org.hl7.fhir.api/ResearchSubject/_search`
- `POST /{tenantId}/cds-{jurisdiction}/v1/{sector}/digitaltwin/org.hl7.fhir.r4/ResearchSubject/_search`

This route is section-first:

- the client sends the IPS `section` token first,
- then sends resource-scoped coded filters such as:
  - `MedicationStatement.code`
  - `Observation.code`
  - `AllergyIntolerance.clinical-status`
- free text, narrative and display claims are removed from the research projection
- the backend may match against leaf indexed resources internally,
  but the public result returns matched `ResearchSubject` aggregates, not leaf
  `MedicationStatement` or `Observation` resources. Each ResearchSubject
  exposes `composition`, its canonical internal Composition index document.

Current request rules:

- body should carry a FHIR `Parameters` resource,
- `section` is required,
- basic discovery uses `date-from`, optional `date-to`, and non-empty `text`,
- advanced coded discovery may instead use resource-scoped claim filters; one
  resource family per request is supported today,
- polling still uses the existing `_batch-response` path with the same `thid`.

Minimal example:

```json
{
  "thid": "digitaltwin-research-subject-search-001",
  "body": {
...
Supported section/resource matrix and the step-by-step tested flow are
maintained in:
...
- The direct `Bundle/_batch` route documented in this section reflects an older
  or lower-level ingestion shape.
- For v2-oriented developer teaching, the preferred secure exchange model is a
  `Communication` carrying the `Bundle` document, usually via
  `Communication.contentAttachment.data` with base64-encoded payload content.
- Keep this section as a transitional reference for explicit route-level
  ingestion, not as the first mental model for new integrations.
...
Use a FHIR Bundle with `type="document"` when you are submitting a *document payload* intended to be indexed. The canonical shape is a Bundle whose first entry is a `Composition` that references the included resources. In this guide, health/IPS examples are used, but the same document pattern is reusable in other domains.
...
- This section describes a derived bundle-document output shape for emergency viewing.
- It does not replace the canonical summary request contract.
- The target request-side contract is `Subject/$summary`, with `Patient/$summary` only as a compatibility alias for healthcare-facing tooling.
...
Routing and fallback note:

- the backend writes the artifact CID/hash separately from the business
  identifier
- this applies to document-oriented clinical bundles, not to every write into
  confidential storage
- provider origin should be resolved before the write using metadata such as
  `countryAddress` and `makesOffer.category`
- if that upstream resolution is unavailable, the backend falls back to the
  individual-index provider sector and jurisdiction already present in the
  runtime context
- the resulting blockchain channel is a backend routing choice, not a claim
  field exposed to the client
...
This is a ledger-side certification endpoint.

- it does not update the individual's index
- it should be used when the system wants to certify a batch hash/CID on
  blockchain without sending the same bundle through Communication ingestion
- if a future `individual/.../Bundle/_seal` route is added, it should keep the
  same ledger-only contract and still avoid index mutation
...
Notes:
- Preferred online form: `attachments[].data.links[0] = "https://.../signed-form.pdf"`.
- Dropbox direct-download links should use the download variant of the shared URL, for example switching the final query flag from `dl=0` to `dl=1`.
...
- `compat`
  ...
  - Accepts `application/didcomm-plain+json` only when `DIDCOMM_PLAIN=enabled|true`
  - Bearer must be present and valid when required by route
- `demo`
...
```
# Transport identity and native FHIR

DIDComm `from`, operational JWT `iss`, signing-key `kid` and SMART `sub` are
separate contract fields. In a direct actor flow, `from`, `iss` and `sub` may
all contain the same actor DID; for DCR the device/client DID normally signs
and sends while SMART `sub` remains the person or professional. Legacy FHIR
adapters receive a native `Communication` or a `Bundle` containing it. Do not
copy DIDComm identity fields into FHIR: the HTTP Authorization header proves
the caller, while `Communication.sender` identifies a FHIR business
participant.
