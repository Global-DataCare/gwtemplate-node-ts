...
**License gating (employee seats):** A signed interactive request sets `gdc.employee.licenseRequired=true`. If no employee seat is `available`, including when no pool exists yet, the final polled bundle entry is an Offer (`Employee-license-offer-v1.0`) containing `org.schema.Offer.*` claims instead of creating the employee. After `Order/_batch` materializes the seat, the caller retries creation and then performs `License/_issue`. A separately governed batch importer may omit this per-operation flag and create unlicensed employee identities for later contracting; `MANDATORY_LICENSE_CREATING_MEMBERS=true` remains the deployment-wide override that prohibits that compatibility mode.
