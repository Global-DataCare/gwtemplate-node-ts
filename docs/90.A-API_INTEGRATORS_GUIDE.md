...
- `individualUuid`: stable demo UUID used to derive the individual multibase id
- `individualDid`: portal-style public DID derived from `individualUuid`
  `did:web:globaldatacare.es:<sector>:individual:multibase:<derived>`
    ...
    "data": [{
      ...
      "meta": {
        "claims": {
          ...
          "@type": "template",
          // Claim key normalization:
          // - When '@context' is 'org.schema', you may send contextualized keys like 'Organization.identifier.value' or 'Service.category'.
          // - This is the documented default mode. If the service enables `CLAIMS_IDENTITY_STORAGE_MODE=canonical`,
          //   the same logical claims may also be sent as fully-qualified `org.schema.*` keys.
          // - The gateway normalizes claims deterministically before downstream storage/processing.
          // - Claims are stored in a canonical (alphabetical) key order to support future deterministic hashing.
          // - In the signed-PDF flow, `Organization.owner.identifier.value` comes from the signer certificate `serialNumber`
          //   (for example `IDCES-...`), not from the controller email.
          // - For `individual/org.schema/Organization`, the semantic split is:
          //   - `Organization.*`: indexed subject
          //   - `Organization.owner.*`: operational binding/contact of the controller
          //   - `Person.*`: controller identity attributes
          //   - `Order.*`: service acceptance metadata
