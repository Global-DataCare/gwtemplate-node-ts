...
- When a communication embeds payload data, the canonical claim is
...
- Prefer canonical `Communication.contentdata` in `resource.meta.claims` as the
  first teaching model.
- Versioned `contentAttachment.data` examples should be presented as projection
  or compatibility shapes around that canonical claim.
- Treat direct resource-specific ingestion routes as compatibility or
  lower-level runtime paths unless the example is explicitly about those routes.
...
- In the v2 model, the relevant document or composition payload should travel
  through `Communication`, typically as an attached base64 artifact in
  `Communication.contentAttachment.data`.
