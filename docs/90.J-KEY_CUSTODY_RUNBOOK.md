...
These purpose names are private KMS routing labels. They are never published
as JWK members. Public JWKs use the registered JOSE `use` values `sig` and
`enc`; DID documents distinguish the communication signer under
`authentication`, credential signers under `assertionMethod`, and encryption
keys under `keyAgreement`.
