...
Fabric stores only the governed state, hash or evidence assigned to the
permissioned ledger. This is not publication of a product in Pontus-X.
...
### 1.3 Product publication in Pontus-X

The organization controller publishes the organization's products, datasets or
service offerings through the `globaldatacare.es` portal and its BFF. Pontus-X
is the external federated catalog and discovery surface for that flow.

GW CORE and dataspace ICA do not reproduce that platform in the local host
deliverable. Their DCAT/DSP surfaces remain technical discovery surfaces, and
their Fabric integration remains the permissioned evidence/state plane.
