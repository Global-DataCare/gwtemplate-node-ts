...
*   **`FabricSponsorshipManager` (at `Org A`):**
    ...
    *   **Logic:**
        ...
        4.  **On Insufficient LoA:** Updates the request status to `PendingStepUpVerification` and initiates the Out-of-band (OOB) flow.
...
*   **New Manager: `FabricActivationManager` (at `Org A`)**
    ...
    *   **Logic:**
        ...
        2.  If valid, it submits the signed Host VC and Root governance decision
            to the privileged Fabric infrastructure reconciler.
        3.  The reconciler registers bounded Fabric CA identities; the
            one-time enrollment material is encrypted for `Controller T`.
        4.  It updates the request status to `Active`.
        5.  The final JWE is returned as the result of this job. The host then
            runs `fabric-ca-client enroll` locally so its MSP/TLS private keys
            and CSRs never leave the host.
