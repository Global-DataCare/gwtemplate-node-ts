...
| Emergency Consent / break-glass | Implemented `ETREAT` Consent period, hash-minimized Fabric anchor, controller-mailbox Communication and renewable short SMART authorization | Live Fabric/mailbox evidence requires configured deployment adapters | Explicit early-revoke endpoint remains extension work | `break_glass`, `emergency_consent_id`, `break_glass_authorization_id` |
...
| Composition/Bundle search (FHIR) | Implemented ingestion and `_search` | Implemented (`Bundle/_search` submit+poll and `Subject/$summary`) | Implemented section updates through `updateClinicalSection`: one `Bundle.type=batch` may mix typed `POST` and exact authored `DELETE ResourceType/id` entries; successful deletion removes the fact from later summaries | `composition.search`, `index.references.upsert` |
| IPS/Unified document and digital-twin projection | Implemented: one canonical Composition per IPS document/version; GW creates the registered `urn:uuid` twin only after canonical secondary-use permit | Implemented: `ResearchSubject/_search` with FHIR Parameters for discovery and saved selections; each ResearchSubject exposes its internal canonical Composition index; same-tenant and bilateral inter-tenant SMART access; basic search across one/many sections by inclusive clinical-date range. Private `__digitalTwinSearch.*` properties coexist on the same projected resource record, not a separate collection, and are stripped from results; `ResearchSubject/$summary` remains public | Implemented: Node SDK uses `researchUseReference`; Consent status reads through Communication -> individual `Subject/_search`; GW owns Consent ids, permit/deny synchronization and provider-offboarding link purge; bilateral Contract VCs name authorized signatories and require both `contractAgreement` proofs; direct canonical batch and invented UUIDs are rejected | Age-range and host-wide aggregation are explicitly post-MVP |
