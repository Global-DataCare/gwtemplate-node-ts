...
| IPS/Unified document and digital-twin projection | Implemented: one Composition per IPS document/version; GW creates the registered `urn:uuid` twin only after canonical secondary-use permit | Implemented: coded twin search, tagged working selections and `ResearchSubject/$summary` materialization | Implemented: the Node SDK identifies each portal/software/study permission with `researchUseReference`; GW resolves or creates its private FHIR Consent identifier, exposes reversible permit/deny synchronization, and provider-offboarding purge removes only the private subject↔twin link; source references keep study consents separate; direct canonical batch and invented UUIDs are rejected | No missing MVP method; executable GW + Node SDK 101 tests are the acceptance evidence |
