...
| IPS/Unified document and digital-twin projection | Implemented: one Composition per IPS document/version; GW creates the registered `urn:uuid` twin only after canonical secondary-use permit | Implemented: same-tenant and bilateral inter-tenant SMART access; basic search across one/many sections by inclusive clinical-date range. Private `__digitalTwinSearch.*` properties coexist on the same projected resource record, not a separate collection, and are stripped from results; tagged selections and `ResearchSubject/$summary` materialization remain public | Implemented: Node SDK uses `researchUseReference`; GW owns Consent ids, permit/deny synchronization and provider-offboarding link purge; bilateral Contract VCs name authorized signatories and require both `contractAgreement` proofs; direct canonical batch and invented UUIDs are rejected | Age-range and host-wide aggregation are explicitly post-MVP |
