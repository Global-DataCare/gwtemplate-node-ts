...
- `artifacts/api-integrators-guide.flow-report.json` is generated by the onboarding flow runner script (`npm run docs:flow-report`) to validate the guide’s end-to-end journey against the running Express app.
  ...
  - It includes intentional negative-control steps such as `2.1.2 Initial Access Token Exchange (invalid code)`; that entry is expected to fail and verifies error handling before the valid exchange step.
...
- Keep the payload strict FHIR for the selected profile/version.

Note:
- Do not mix interoperable claims into strict FHIR resources.
- Claims-first payloads go in `body.data[].meta.claims`.
- Strict FHIR payloads go in `body.entry[].resource`.
