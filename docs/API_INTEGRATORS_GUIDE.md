...
## ICA-Signed X.509 for Tenant Connectors (gwtemplate flow)

This platform requires **tenant connectors** to hold an **ICA-signed X.509 certificate** while **private keys stay local** in the tenant’s `gwtemplate`. The ICA signs a CSR, and the signed certificate is then attached to the tenant’s public metadata (JWK/DID/x509 endpoint).
...
1. **Key pair & CSR (tenant/gwtemplate):**  
   ...
   - `OID 2.5.4.97 (organizationIdentifier) = TAXES-<country>-<taxId>`
...
**test ICA** and binds the tenant to the **test Fabric namespace**.  
...
- After a tenant registration `Order` is accepted, the backend auto-issues the **first** activation code for the **tenant legal representative** and returns it inside the Order response claims:
  - `Order.activationCode` (the activation code)
  - `Order.activationCategory = "professional"` (disambiguation)
- After a family registration `Order` is accepted, the backend auto-issues the **first** activation code for the **family controller** and returns it inside the Order response claims:
  - `Order.activationCode` (the activation code)
  - `Order.activationCategory = "individual"` (disambiguation)
...
### DocumentReference-first ingestion (strict FHIR)

For clinical document uploads (PDF/JPG/PNG + metadata), use `DocumentReference` as the registration/indexing resource:
...
- Claims namespace: `DocumentReference.*` only (FHIR-aligned)

Recommended sequencing:
1. Register one or more documents with `DocumentReference`.
2. Optionally send `Communication` for delivery/notification (referencing already-registered artifacts).

This keeps "clinical indexing" and "message exchange" as separate concerns.
...
- Use `individual/org.hl7.fhir.api/Communication/_batch` when you intentionally exchange version-agnostic interoperable meta-claims (not tied to a specific FHIR release, e.g., onboarding-style data modeled as generic claims).
