...
- **Responsabilidades:**
  - Mantener un registro de las identidades de los tenants participantes (DIDs).
  ...
  - Proporcionar la función `getNotificationEndpoint(individualIdentifier)` para ser consultada por los Productores de Datos.
...
2.  **Formalización del Consentimiento:** El consentimiento puede formalizarse de varias maneras:
    *   **Firma Digital:** El Individuo firma digitalmente el PDF de los T&C.
    *   **Firma Manuscrita:** El Individuo firma el documento en persona (por ejemplo, en la recepción de una clínica). Un administrativo digitaliza el documento.
...
4.  **Procesamiento y Activación:**
    *   El sistema recibe y valida el documento de consentimiento.
    ...
    *   Finalmente, Tenant C invoca el smart contract en la Red de Confianza para registrar el endpoint de notificación del Individuo: `registerNotificationEndpoint(individualIdentifier, tenant_C_notification_url)`.

A partir de este momento, el índice del Individuo está activo y cualquier Productor de Datos en la red puede descubrir a dónde debe enviar las notificaciones.
...
1.  **Generación de Datos:** Tenant D, un hospital, genera un informe de alta para el paciente. El informe se almacena en su sistema como un recurso FHIR `DocumentReference` y se le asigna una URL de acceso segura y estable.

2.  **Descubrimiento del Endpoint:** El sistema de Tenant D, antes de enviar una notificación por SMS o email, consulta la Red de Confianza.
    *   Llama a la función del smart contract: `getNotificationEndpoint(individualIdentifier)`.
    *   El smart contract devuelve la URL del endpoint de notificaciones registrado por Tenant C para ese Individuo.

3.  **Construcción de la Notificación:** Si se devuelve una URL, Tenant D construye un `Bundle` de FHIR de tipo `message`.
...
4.  **Envío de la Notificación:** Tenant D envía este `Bundle` mediante una petición `HTTP POST` al endpoint de Tenant C.

5.  **Actualización del Índice:** Tenant C recibe y valida el mensaje. Extrae la información del `DocumentReference` y la añade a la sección correspondiente del `Composition` del Individuo. El índice de salud del paciente está ahora actualizado en tiempo real.
...
1.  **Consulta del Índice:** Un médico (que trabaja para Tenant E) utiliza una aplicación para ver el índice de salud unificado del paciente, alojado y servido por Tenant C. El médico ve una entrada para un "Informe de Alta del Hospital D" y hace clic para abrirlo.
...
3.  **Emisión del Bearer Token:** Tenant C, actuando como Servidor de Autorización delegado, verifica que el médico está autorizado para ver los datos de este paciente. Si la autorización es válida, emite un **JSON Web Token (JWT)** con las siguientes `claims`:
    ...
    *   `scope`: Define el permiso granular. Puede ser:
        - **Específico a un recurso:** `patient/DocumentReference.read?_id=informe-alta-123`
        - **Específico a una sección:** `patient/Observation.read?category=laboratory` (para acceder a todos los resultados de laboratorio).

4.  **Recuperación del Dato:** La aplicación del médico realiza una petición `HTTP GET` a la URL del recurso (que obtuvo del `DocumentReference` en el índice), incluyendo el token en la cabecera:
    ```
    GET /fhir/DocumentReference/informe-alta-123 HTTP/1.1
    Host: api.tenant-d.com
    Authorization: Bearer <JWT_EMITIDO_POR_TENANT_C>
    ```

5.  **Validación y Respuesta:** El servidor de Tenant D:
    *   Recibe la petición.
    *   Valida la firma del JWT usando la clave pública de Tenant C.
    *   Verifica que la `aud` (audiencia) se corresponde con él mismo.
    *   Verifica que el `scope` autoriza la acción solicitada.
    *   Si todo es correcto, devuelve el recurso FHIR completo.

Este mecanismo garantiza que los datos nunca abandonan el sistema del productor sin una prueba criptográfica de autorización granular y específica, emitida por el agente de confianza del paciente.
